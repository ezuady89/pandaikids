...
export default function SemakAktivitiPage() {
  ...
  const publishQuiz = async () => {
    ...
    window.localStorage.setItem("pandaikids-cikgu-display-name", cleanTeacherName);
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.shell}>
      ...
      <section className={styles.publish}>
        ...
        <div className={styles.publishActions}>{quizUrl ? <><a href="/dashboard/">Dashboard Saya</a><a href={quizUrl} target="_blank" rel="noreferrer">Lihat sebagai murid ↗</a></> : null}<button disabled={!canEdit || publishing} onClick={publishQuiz}>{publishing ? "Menyimpan…" : !authenticated ? "Log masuk & terbitkan" : quizId ? "Simpan & salin pautan" : "Terbitkan & salin pautan"} <b>→</b></button></div>
      </section>
    </section>
  </main>;
}
