      ...
      {mode === "choose" ? <div className={styles.modeGrid}>
        ...
        <section className={styles.workspace}>
          ...
          {mode === "material" ? <div className={styles.materialPanel}>
            <div className={styles.materialTop}><div><small>AI BANTU CIKGU</small><h2>AI hasilkan soalan secara automatik.</h2></div><label>Bilangan<select value={count} onChange={(event) => setCount(Number(event.target.value))}>{[5,10,15,20].map((item) => <option key={item} value={item}>{item} soalan</option>)}</select></label></div>
