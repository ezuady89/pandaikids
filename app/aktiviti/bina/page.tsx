...
export default function BinaKuizPage() {
  ...
  const generateQuestions = async () => {
    ...
    try {
      ...
      setQuestions(generated); setActive(0); setMode("manual");
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.shell}>
      <div className={styles.intro}><span>BINA IKUT CARA CIKGU</span><h1>Bina kuiz sendiri.</h1><p>Masukkan sendiri atau jadikan bahan cikgu sebagai kuiz.</p></div>

      {mode === "choose" ? <div className={styles.modeGrid}>
        <button type="button" onClick={() => setMode("manual")}><i>✎</i><small>CARA 1</small><h2>Masukkan Soalan Sendiri</h2><p>Isi soalan, jawapan A–D dan tandakan jawapan betul.</p><b>Mula isi <span>→</span></b></button>
        <button type="button" onClick={() => setMode("material")}><i>✦</i><small>CARA 2</small><h2>Hasilkan Daripada Bahan</h2><p>Gunakan tajuk, teks, nota bergambar atau fail PDF.</p><b>Gunakan bahan <span>→</span></b></button>
      </div> : <>
        <div className={styles.modeSwitch}><button className={mode === "manual" ? styles.active : ""} onClick={() => setMode("manual")}>Masukkan sendiri</button><button className={mode === "material" ? styles.active : ""} onClick={() => setMode("material")}>Daripada bahan</button></div>
        <section className={styles.workspace}>
          ...
          {mode === "material" ? <div className={styles.materialPanel}>
            <div className={styles.materialTop}><div><small>BAHAN CIKGU</small><h2>Hasilkan soalan secara automatik.</h2></div><label>Bilangan<select value={count} onChange={(event) => setCount(Number(event.target.value))}>{[5,10,15,20].map((item) => <option key={item} value={item}>{item} soalan</option>)}</select></label></div>
            ...
            <div className={styles.uploadRow}><label className={styles.upload}><input type="file" accept=".pdf,.txt,image/jpeg,image/png,image/webp" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><span>＋ Pilih gambar, PDF atau teks</span><small>{file ? file.name : "Maksimum 4 MB"}</small></label><button className={styles.generate} disabled={busy} type="button" onClick={generateQuestions}>{busy ? "Sedang menghasilkan…" : "Hasilkan Soalan"} <span>✦</span></button></div>
