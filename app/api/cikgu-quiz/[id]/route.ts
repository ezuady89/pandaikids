...
type QuizBody = { ownerToken?: string; edits?: Record<string, QuestionEdit>; corrections?: Correction[]; theme?: string };
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      ...
      const owner = await client.query("SELECT owner_token_hash FROM teacher_quizzes WHERE id = $1 FOR UPDATE", [id]);
      ...
      await client.query("UPDATE teacher_quizzes SET question_overrides = $1::jsonb, theme = $2, updated_at = NOW() WHERE id = $3", [JSON.stringify(body.edits ?? {}), body.theme ?? "coral", id]);
